...
const imagesBoxEl = document.querySelector(
  '.review-slider__img-box',
) as HTMLDivElement;
const wrapperEl = document.querySelector('.review-slider__wrapper') as HTMLDivElement;
...
const navBoxEl = document.querySelector('.review-slider__nav-box') as HTMLDivElement;
...
navItemList = Array(imagesLength)
  ...
  .map((_el, index) => {
    ...
    navItemEl.classList.add('review-slider__nav-item');
    if (index === 0) {
      navItemEl.classList.add('review-slider__nav-item_active');
    }
    navItemEl.dataset.image = String(index + 1);
    navItemEl.textContent = String(index + 1);
    navItemEl.addEventListener('click', e => {
      ...
      navItemList[currentImage - 1].classList.add('review-slider__nav-item_active');
      navItemList[prevCurrentImage - 1].classList.remove(
        'review-slider__nav-item_active',
      );
      setTimeout(() => {
...
prevBtnEl.addEventListener('click', () => {
  ...
  navItemList[currentImage - 1].classList.add('review-slider__nav-item_active');
  navItemList[prevCurrentImage - 1].classList.remove('review-slider__nav-item_active');
